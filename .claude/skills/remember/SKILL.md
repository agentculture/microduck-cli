...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope microduck-cli
...
scope:** the default personal scope is **private** (`--scope microduck-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `microduck-cli`). Private records are served only to a recall in the same
