...
| **DGX Spark**, re-test 2026-09-07 following the [Jetson AI Lab tutorial](docs/verification/2026-09-07-spark-retest.md) on the **PyPI 0.9.4 wheel**, viewer **windowed** | tutorial steps 1 and 5–11 executed on the wheel (steps 2–4, clone/build/venv, checked against pre-existing state, not re-run) + both live suites + smoke | pass; **12 passed, 1 xfailed** windowed | the wheel's `env doctor` printed two `[FAIL]` pin lines under a `healthy` verdict (pins file not shipped — fixed in 0.9.6: the pins are packaged and warnings render `[WARN]`); the smoke needs the GPU free — it failed OOM beside a resident vLLM engine and passed with it paused |
